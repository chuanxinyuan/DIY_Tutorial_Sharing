...
/**
 * 加载视图组件的函数
 * @param {string} path - 视图组件的路径
 * @returns {Function} 返回一个异步加载函数，用于动态导入组件
 */
export function loadView(path) {
	/**
	 * 打包成一个 js、一个 css 的注释代码
	 * 这部分代码被注释掉了，它原本用于将组件打包成单个JS和CSS文件
	 */
	// if (path.indexOf('layout') > -1) return () => Promise.resolve(require(`@/${path}`));
	// else return () => Promise.resolve(require(`@/views/${path}`));

	/**
	 * 打包成多个 js、多个 css 的当前实现
	 * 使用动态import()语法，实现组件的按需加载
	 * 根据路径中是否包含'layout'来决定不同的导入路径
	 */
	if (path.indexOf('layout') > -1) return () => import(`@/${path}`);  // 如果路径中包含'layout'，则直接从根目录导入
	else return () => import(`@/views/${path}`);  // 否则从views目录导入
}
...
export function yonghuUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuYonghu()
		.then(async (res) => {
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'yonghu') yonghuUser(router, to, next);
