	...
	methods: {
		...
		getLayoutThemeConfig() {
			if (Local.get('themeConfigPrev')) {
				const cacheTheme = Local.get('themeConfigPrev');
				// 兼容历史缓存中的旧项目名，启动时自动迁移
				if (cacheTheme && cacheTheme.globalTitle === '校园二手商城系统') {
					cacheTheme.globalTitle = '旧物改造教程分享平台';
					cacheTheme.globalViceTitle = '管理员后台';
					Local.set('themeConfigPrev', cacheTheme);
				}
				this.$store.dispatch('themeConfig/setThemeConfig', cacheTheme);
